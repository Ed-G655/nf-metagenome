...
process QA_FILTER {
	...
	less -S $QA | tr -d "#-"  | tr -s " " | tr " " "\t" | cut -f2,8,9 | awk -F "\t" '{ if( \$2 >= 80 && \$3 <= 20 ) { print } }' > $Sample_name'_filtered_bins'
	sed -i "1 s/.*/Bin_id\tCompleteness\tContamination/" $Sample_name'_filtered_bins' > $Sample_name'_filtered_bins.tsv'
